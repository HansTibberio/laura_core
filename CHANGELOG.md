...
- Add `PieceParseError`, `SquareParseError` and `CastleRightsParseError`
- Replace &'static str errors with typed, descriptive enums
- Improve error propagation and diagnostics in Board::from_str
- Implement Display for all parsing error types
...
- Added new functions to the **Square** module:
    - `right_color()` and `left_color()` to determine the direction of adjacent squares based on the color of the side
      to move.
